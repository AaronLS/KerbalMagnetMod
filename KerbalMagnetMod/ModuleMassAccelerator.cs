                ...
                if (armed)
                {
                    if (firetime < accelerationDuration)
                    {
                        if (vLaunchTarget != null)
                        {
                           ...
                           // vLaunchTarget
                                //.rigidbody.AddRelativeForce(Vector3.forward * 10f, ForceMode.
                             //   .ChangeWorldVelocity(accelerateVector * (a * (float)dt));
        ...
        public override void OnUpdate()
        {
            ...
            foreach (Vessel v in vesselList)
            {
               ...
               // print(v.distanceUnpackThreshold);
                ...
                if (Vector3.Distance(myloc, theirloc) < 2000)
                {
                    v.distanceUnpackThreshold = 2000;
