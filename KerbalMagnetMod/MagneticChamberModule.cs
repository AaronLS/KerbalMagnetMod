...
namespace CertifiedSilly.MagneticRings
{
    ...
    public class MagneticChamberModule : PartModule, IChamberCoupler//IMagnet,   // ModuleDockingNode
    {
       ...
       // [KSPEvent(guiActive = true, guiName = "Deactivate Magnet", name = "DeactivateMagnet", guiActiveUnfocused = true,active = false)]
        ...
        [KSPField(isPersistant = true)]
        uint otherDockedVesselRootPartUid;

        

        [KSPEvent(guiActive = true, guiName = "Decouple", name = "Decouple", category = "aaaToggleMagnet")] 
        public void Decouple()
        {
            var dockedVesselRootPart = (from p in vessel.parts where p.flightID == otherDockedVesselRootPartUid select p).SingleOrDefault();
            dockedVesselRootPart.decouple();
            //otherVesselRootPart.decouple();
        }

        // https://github.com/taniwha-qf/Extraplanetary-Launchpads/blob/master/Source/BuildControl.cs
        private void CoupleWithCraft(Vessel otherVessel)
        {
            Part otherVesselRootPart;
            DockedVesselInfo otherDockedVessel = new DockedVesselInfo();
            otherDockedVessel.name = otherVessel.vesselName;
            otherDockedVessel.vesselType = otherVessel.vesselType;
            otherDockedVessel.rootPartUId = otherVessel.rootPart.flightID;
            //otherVesselRootPart = otherVessel.rootPart;
            otherVessel.rootPart.Couple(this.part);
            otherDockedVesselRootPartUid = otherVessel.rootPart.flightID;
